...
import { type DripAuthValue, dripAuthSchema, dripErrorSchema } from "./schemas"
...
export const getDripClient = (auth: DripAuthValue) => {
...
export async function dripRequest<T>(
  authValue: DripAuthValue,
  path: string,
  schema: z.ZodType<T>,
  options?: Options,
): Promise<T> {
  const auth = dripAuthSchema.parse(authValue)
