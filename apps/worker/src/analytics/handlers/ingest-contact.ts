...
const ingesterService = new ClickhouseIngester({
  ...
  clickhouseTable: CONTACT_EVENTS_EVENT_TYPE,
  batchSize: clickhouseConfig.batchSize,
  maxRetries: clickhouseConfig.maxRetries,
})

export const ingestContactEvents = async (_job: unknown) => {
  logger.info("Starting contact events ingestion to ClickHouse")

  try {
    await ingesterService.ingestCommittedFiles()
    logger.info("Contact events ingestion completed")
  } catch (error) {
    logger.error("Contact events ingestion failed", error)
    throw error
  }
}
