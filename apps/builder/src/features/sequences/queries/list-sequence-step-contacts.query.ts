...
export async function listSequenceStepContacts(input: {
  chatbotId: string
  sequenceId: string
  stepId: string
  ...
  page: number
  perPage: number
}): Promise<ListSequenceStepContactsResponse> {
  await assertCurrentUserCanAccessChatbot(input.chatbotId)

  const { chatbotId, sequenceId, stepId, eventType, page, total, perPage } =
    input
  ...
  const data: SequenceStepContactData[] = contactRows
    .map((row) => {
