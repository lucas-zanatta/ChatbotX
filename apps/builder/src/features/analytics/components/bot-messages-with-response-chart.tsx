...
interface BotMessagesWithResponseChartProps {
  chatbotId: string
}

export function BotMessagesWithResponseChart({
  chatbotId,
}: BotMessagesWithResponseChartProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/chatbots/${chatbotId}/analytics/bot-messages-with-response?${params}`,
        )
        ...
        console.error("Failed to fetch bot messages with response:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  return <BarChart data={data} name={t("analytics.botMessagesWithResponse")} />
}
