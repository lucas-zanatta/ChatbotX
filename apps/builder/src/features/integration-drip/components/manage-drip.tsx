...
export function ManageDrip(props: {
  ...
  const featureName = t("fields.drip.label")
  const { form, handleSubmitWithAction } = useHookFormAction(
    connectDripAction.bind(null, props.workspaceId),
    zodResolver(connectDripSchema),
    {
      actionProps: {
        onSuccess: () => {
          setOpen(false)
      ...
      formProps: {
        mode: "onChange",
        defaultValues: { apiToken: "", accountId: "" },
      },
    },
  )
  const { execute: disconnect, isPending } = useAction(
    disconnectDripAction.bind(null, props.workspaceId),
    {
      onSuccess: () => {
        router.refresh()
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <Form {...form}>
              <form className="space-y-4" onSubmit={handleSubmitWithAction}>
                <InputField
                  label={t("drip.fields.accountId")}
                  name="accountId"
                  required
                />
                <InputField
                  label={t("drip.fields.apiToken")}
                  name="apiToken"
                  required
                  type="password"
                />
