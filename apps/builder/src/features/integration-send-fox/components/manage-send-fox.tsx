...
import { connectSendFoxAction } from "../actions/connect.action"
import { disconnectSendFoxAction } from "../actions/disconnect.action"
import { connectSendFoxSchema } from "../schemas"

export function ManageSendFox(props: {
  workspaceId: string
  isConnected: boolean
}) {
  ...
  const featureName = t("fields.sendFox.label")
  const { form, handleSubmitWithAction } = useHookFormAction(
    connectSendFoxAction.bind(null, props.workspaceId),
    zodResolver(connectSendFoxSchema),
    {
      actionProps: {
        onSuccess: () => {
          setOpen(false)
      ...
      formProps: { mode: "onChange", defaultValues: { accessToken: "" } },
    },
  )
  const { execute: disconnect, isPending } = useAction(
    disconnectSendFoxAction.bind(null, props.workspaceId),
    {
      onSuccess: () => {
        router.refresh()
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <Form {...form}>
              <form className="space-y-4" onSubmit={handleSubmitWithAction}>
                <InputField
                  label={t("sendFox.fields.accessToken")}
                  name="accessToken"
                  required
                  type="password"
                />
